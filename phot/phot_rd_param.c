/* Function Package : phot_rd_param */
/* Version: 0.1.0 */
...
int rd_apphot_param( FILE *istream, APPHOTPARS *pars )
{
 ...
 while (fgets( line, 2*MAXLENGTH, istream ) != NULL) {
   if ( !strncmp(line,"apphot.image=",13) ) {
     pstr = line+13;
     status = phot_assign_str_param( pstr, tmp );
     if (strlen(tmp) != 0) {
     ...
     pstr = line+15;
     status = phot_assign_str_param( pstr, pars->apphot_imglist );
   }
   else if ( !strncmp(line,"apphot.coords=",14) ) {
     pstr = line+14;
     status = phot_assign_str_param( pstr, pars->apphot_coords );
   }
   else if ( !strncmp(line,"apphot.output=",14) ) {
     pstr = line+14;
     status = phot_assign_str_param( pstr, pars->apphot_output );
     ...
     pstr = line+16;
     status = phot_assign_str_param( pstr, pars->apphot_datapars );
     ...
     pstr = line+18;
     status = phot_assign_str_param( pstr, pars->apphot_fitskypars );
...
int rd_data_param( FILE *istream, DATAPARS *pars )
{
 ...
 while (fgets( line, 2*MAXLENGTH, istream ) != NULL) {
   if ( !strncmp(line, "datapars.scale=",15) ) {
     pstr = line+15;
     if (status = phot_assign_double_param( pstr, &pars->data_scale )) {
     ...
     pstr = line+19;
     status = phot_assign_str_param( pstr, pars->data_hexposure );
   }
   else if ( !strncmp(line, "datapars.hairmass=",18) ) {
     pstr = line+18;
     status = phot_assign_str_param( pstr, pars->data_hairmass );
   }
   else if ( !strncmp(line, "datapars.itime=",15) ) {
     pstr = line+15;
     if (status = phot_assign_double_param( pstr, &pars->data_itime )) {
     ...
     pstr = line+14;
     if (status = phot_assign_double_param( pstr, &pars->data_gain )) {
...
int rd_fitsky_param( FILE *istream, FITSKYPARS *pars )
{
 ...
 while (fgets( line, 2*MAXLENGTH, istream ) != NULL) {
   if ( !strncmp(line, "fitskypars.algorithm=",21) ) {
     pstr = line+21;
     status = phot_assign_str_param( pstr, pars->fitsky_algorithm );
   }
   else if ( !strncmp(line, "fitskypars.annulus=",19) ) {
     pstr = line+19;
     if (status = phot_assign_double_param( pstr, &pars->fitsky_annulus )) {
     ...
     pstr = line+20;
     if (status = phot_assign_double_param( pstr, &pars->fitsky_wannulus )) {
     ...
     pstr = line+20;
     if (status = phot_assign_double_param( pstr, &pars->fitsky_skyvalue )) {
     ...
     pstr = line+19;
     if (status = phot_assign_int_param( pstr, &pars->fitsky_maxiter )) {
     ...
     pstr = line+21;
     if (status = phot_assign_int_param( pstr, &pars->fitsky_maxreject )) {
     ...
     pstr = line+17;
     if (status = phot_assign_int_param( pstr, &pars->fitsky_nkeep )) {
     ...
     pstr = line+20;
     if (status = phot_assign_double_param( pstr, &pars->fitsky_loreject )) {
     ...
     pstr = line+20;
     if (status = phot_assign_double_param( pstr, &pars->fitsky_hireject )) {
       fprintf(stderr,ERRMSG_MISSING_INPUT,"fitskypars.hireject");
       return(status);
     }
   }
   else { return(phot_warn_unknown_param(line)); }
 }
...
int rd_phot_param( FILE *istream, PHOTPARS *pars )
{
 ...
 while (fgets( line, 2*MAXLENGTH, istream ) != NULL) {
   if ( !strncmp(line, "photpars.ashape=",16) ) {
     pstr = line+16;
     status = phot_assign_str_param( pstr, pars->phot_ashape );
   }
     ...
     pstr = line+15;
     if (status = phot_assign_double_param( pstr, &pars->phot_ellip )) {
     ...
     pstr = line+12;
     if (status = phot_assign_double_param( pstr, &pars->phot_pa )) {
     ...
     pstr = line+14;
     if (status = phot_assign_double_param( pstr, &pars->phot_zmag )) {
...
/* Function : phot_assign_double_param */
/* Version: 0.1.0 */
/*   Parse and assign a double precision parameter
 *
 * int phot_assign_double_param( char *pstr, double *param )
 *
 * Usage: status = phot_assign_double_param( pstr, &param )
 */
int phot_assign_double_param( char *pstr, double *param )
{
 int status=0;
...
/* Function : phot_assign_str_param */
/* Version: 0.1.0 */
/*   Parse and assign a string parameter
 *
 * int phot_assign_str_param( char *pstr, char *param )
 *
 * Usage: status = phot_assign_str_param( pstr, param );
 */
int phot_assign_str_param( char *pstr, char param[] )
{
 int l,status=0;
...
/* Function : phot_warn_unknown_param */
/* Version: 0.1.0 */
/*   Warn of unknown parameter in a param file
 *
 * int phot_warn_unknown_param( char line[] )
 *
 * Usage: status = phot_warn_unknown_param(line);
 */
