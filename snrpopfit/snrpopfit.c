...
char *argv[];
{
 ...
 for (ll=0,jj=pars.snrpopfit_section[2];jj<pars.snrpopfit_section[3];ll++,jj++) {
   for (kk=0,ii=pars.snrpopfit_section[0];ii<pars.snrpopfit_section[1];kk++,ii++) {
 ...
 for (index=start_index,isim=1;isim<=nsim;index++,isim++) {
    ...
    for (p=0; p<pars.snrpopfit_Nnsnr; p++) {
      ...
      sprintf(simpop_command,"nice -19 %s %s -o %s --nsnr=%g --snrate=%g --nambient=%g",
 		   MKSNRPOP,pars.snrpopfit_mksnrpop,OUTPOP,nsnr,snrate,nambient);
      sprintf(simimg_command,"nice -19 %s %s -o %s --inputpop=%s",
	...
	strcat(simimg_command,pars.snrpopfit_addpop);
      ...
      dchisqimg(preal, psim, naxes, pars.snrpopfit_section, pstddev, &chi2, &prob);
      *(*(*(pchi2+r)+q)+p) = chi2;
      *(*(*(pprob+r)+q)+p) = prob;
